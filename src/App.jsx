...
function App() {
  ...
  useEffect(() => {
    // Load dynamic content from Firestore if available
    fetchSiteConfig().then((cfg) => {
      if (!cfg) return
      if (cfg.tools) setTools(cfg.tools)
      if (cfg.projects) setProjects(cfg.projects)
      if (cfg.heroTitle) setHeroTitle(cfg.heroTitle)
      if (cfg.heroQuote) setHeroQuote(cfg.heroQuote)
      if (cfg.aboutText) setAboutText(cfg.aboutText)
      if (cfg.heroSmallAvatar) setHeroSmallAvatar(cfg.heroSmallAvatar)
      if (cfg.cvHref) setCvHref(cfg.cvHref)
      if (cfg.cvDownloadName) setCvDownloadName(cfg.cvDownloadName)
      if (cfg.profile) setProfile((p) => ({ ...p, ...cfg.profile }))
    }).catch(() => {})
  }, []);
        ...
        <div className="hero grid md:grid-cols-2 items-center pt-10 xl:gap-0 gap-6 grid-cols-1">
          <div className="animate__animated animate__fadeInUp animate__delay-3s">
            <div className="flex items-center gap-3 mb-6 bg bg-zinc-800 w-fit p-4 rounded-2xl">
              <img src={heroSmallAvatar} className="w-10 rounded-md" />
              <q>{heroQuote}</q>
            </div>
            <h1 className="text-5xl font-bold mb-6">
              <ShinyText text={heroTitle} disabled={false} speed={3} className='custom-class' />
            </h1>
            <BlurText
              text={aboutText}
              delay={150}
